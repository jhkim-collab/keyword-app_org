    ...
    for r in records:
        ...
        if match_mode == "유사 키워드 포함":
            for v in variants:
                ...
                if (not starts) and bool(kwargs.get("similar_reverse_single", False)) and len(tokens_lower) == 1:
                    q = tokens_lower[0].strip() if tokens_lower else ""
                    if q:
                        # Match if the query token is contained in any keyword token (case-insensitive).
                        # This aligns with 'spot' -> 'dark spot', 'dark spots', 'spot-prone', 'hotspot', etc.
                        if any((q in t) for t in v_tokens_lower):
                            starts = [0]
                            try:
...
""",
    ...
    <div>
      ...
      <style>
        ...
        .hl-scroll {{ max-height: 360px; overflow: auto; line-height: 1.6; }}
        ...
        #kwd-tt-float table {{ border-collapse: collapse; width: 100%; font-size: 12px; }}
...
.hl-box .hl-riskN{background:#ffd43b !important;background-color:#ffd43b !important;}
      ...
      <script>
        (function(){{
          ...
          function showFor(target){{
            ...
            if(y + bh + 8 > vh) y = Math.max(8, rect.top - bh - 8);
    ...
    components.html(_component_html, height=420, scrolling=True)
